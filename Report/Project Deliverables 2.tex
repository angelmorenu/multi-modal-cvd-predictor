...
\begin{figure}[!t]
    \centering
    % Left: SHAP / feature importance image with fallbacks
    \IfFileExists{\detokenize{figures/shap_force_example.png}}{%
        \includegraphics[width=0.48\columnwidth]{\detokenize{figures/shap_force_example.png}}%
    }{%
        \IfFileExists{\detokenize{figures/shap_force_example.pdf}}{%
            \includegraphics[width=0.48\columnwidth]{\detokenize{figures/shap_force_example.pdf}}%
        }{%
            \fbox{%
                \parbox[b][4cm][c]{0.48\columnwidth}{\centering SHAP / feature importance image not found\\(expected: figures/shap\_force\_example.png or .pdf)}%
            }%
        }%
    }
    % Right: ECG saliency image with fallbacks
    \IfFileExists{\detokenize{figures/ecg_saliency.png}}{%
        \includegraphics[width=0.48\columnwidth]{\detokenize{figures/ecg_saliency.png}}%
    }{%
        \IfFileExists{\detokenize{figures/ecg_saliency.pdf}}{%
            \includegraphics[width=0.48\columnwidth]{\detokenize{figures/ecg_saliency.pdf}}%
        }{%
            \fbox{%
                \parbox[b][4cm][c]{0.48\columnwidth}{\centering ECG saliency image not found\\(expected: figures/ecg\_saliency.png or .pdf)}%
            }%
        }%
    }
    \caption{Interpretability examples. Left: tabular feature importance (SHAP or RandomForest fallback). Right: ECG saliency map (derivative-based or Captum).}
    \label{fig:interpret}
\end{figure}

...
\begin{enumerate}
    \item \textbf{Multi-Modal Fusion Architecture:} I design and validate a neural network that fuses 1D-CNN ECG embeddings with tabular feature embeddings, demonstrating that joint modeling can capture synergies across modalities.
    
    \item \textbf{Robust Preprocessing and Data Handling:} I implement defensive preprocessing with automatic shape alignment, missing-value imputation, and per-signal normalization, enabling reproducible pipelines even with small datasets and heterogeneous feature sets.
    
    \item \textbf{Calibration and Uncertainty Quantification:} I apply Platt scaling post-hoc to improve probability calibration, addressing a critical need for trustworthy risk estimates in clinical practice.
    
    \item \textbf{Interpretability and Explainability:} I integrate SHAP-based feature importance, ECG saliency visualization, and fallback RandomForest explanations, ensuring that clinicians can understand model decisions.
    
    \item \textbf{Interactive and Auditable User Interface:} I develop a Streamlit-based interface that logs predictions with full audit trails, enabling reproducibility and regulatory compliance.
    
    \item \textbf{Responsible AI Considerations:} I reflect on fairness, privacy, and transparency throughout the system design and document limitations and future work for clinical deployment.
\end{enumerate}

\subsection{Scope and Report Organization}
This report synthesizes the complete project lifecycle: from initial prototyping (Deliverables 2--3) to a polished, research-grade system (Deliverable 4). I structure the report as follows: Section~\ref{sec:related} surveys related work and highlights my novel contributions; Section~\ref{sec:design} describes system architecture and implementation; Section~\ref{sec:evaluation} presents quantitative results and visual diagnostics; Section~\ref{sec:discussion} reflects on findings, limitations, and insights; Section~\ref{sec:future} outlines future research directions; Section~\ref{sec:responsible_ai} addresses responsible AI; and Section~\ref{sec:conclusion} concludes with key takeaways.
...
I integrate three publicly available datasets to construct a multi-modal representation of cardiovascular health:
...
\textbf{PTB-XL ECG Database (Kaggle):} The PTB-XL database is one of the largest publicly available ECG datasets, containing 21,837 clinical 12-lead ECG recordings sampled at 100~Hz (with 500~Hz versions also available). Each recording is 10 seconds in duration and includes expert annotations for rhythm, conduction abnormalities, and myocardial infarction patterns. I use the 100~Hz version to balance signal fidelity with computational efficiency. The 12-lead configuration provides spatial information about cardiac electrical activity across different anatomical planes.

\subsubsection{Data Integration and Alignment Strategy}
Integrating heterogeneous datasets presents challenges related to identifier mapping, temporal alignment, and missingness. My integration pipeline operates as follows:

\begin{enumerate}
    \item \textbf{Synthetic Patient Identifier Assignment:} Since the three datasets originate from different sources without common patient identifiers, I construct a synthetic cohort by sampling records from each dataset and assigning pseudo-identifiers. In a clinical deployment, this would correspond to linking electronic health records (EHRs), administrative claims, and diagnostic testing databases via unique patient identifiers (e.g., medical record numbers).
    ...
    \item \textbf{Temporal Considerations:} Although my proof-of-concept treats data as cross-sectional, I document timestamp fields for future longitudinal modeling. In practice, ECG recordings, tabular risk assessments, and hospital admissions would be temporally ordered and potentially asynchronous.
...
\begin{enumerate}
    ...
    \item \textbf{Lead Selection:} While the full 12-lead system is available, my proof-of-concept focuses on lead II for simplicity. Future work will leverage all 12 leads via multi-channel 1D CNNs.
...
\textbf{Class Imbalance:} CVD prevalence in the integrated dataset is approximately 20\%, creating imbalanced train/validation/test splits. I address this through:
\begin{itemize}
    ...
    \item \textbf{Threshold Tuning:} At inference, I adjust the classification threshold (default 0.5) to balance sensitivity and specificity according to clinical cost-benefit analysis.
...
\textbf{Data Quality and Annotation Noise:} Public datasets may contain labeling errors, inconsistent annotations, or selection biases. I acknowledge these limitations and recommend clinical validation with gold-standard expert annotations for deployment.
...
I apply \textbf{Platt Scaling}, a post-hoc calibration technique that fits a logistic regression model on validation set predictions:
\[
...
Platt scaling is lightweight (only two parameters), requires minimal data, and demonstrably improves calibration metrics (Brier score, expected calibration error) as shown in my results.

\subsubsection{Evaluation Metrics}
I employ a comprehensive suite of metrics to assess different aspects of model performance:
...
Interpretability is paramount in clinical AI systems, where clinicians and patients must understand and trust model predictions. I implement multiple complementary interpretability strategies to provide both global (model-level) and local (instance-level) explanations.
...
\begin{itemize}
    \item I use the KernelExplainer from the \texttt{shap} library, which is model-agnostic and works with any black-box classifier.
...
For deep learning models processing time-series or image data, gradient-based saliency maps highlight influential input regions. I compute saliency for ECG signals as:
\[
...
SHAP and gradient-based methods require additional dependencies (\texttt{shap}, \texttt{captum}) that may not be available in all deployment environments (e.g., resource-constrained edge devices, air-gapped clinical systems). To ensure explanations are always available, I implement lightweight fallbacks:
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Confidence Intervals:} For calibrated models, I display 95\% confidence intervals derived from bootstrapping or Bayesian approximations (future work).
...
Healthcare AI systems risk perpetuating or amplifying existing health disparities if not carefully designed and audited. I adopt a multi-faceted approach to fairness:
